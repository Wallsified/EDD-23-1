...
public abstract class Heap<T extends Comparable<T>> implements Coleccionable<T> {
  ...
  protected void reordenaParaAbajo(int n, boolean esMin) {
    int indiceIzquierdo = izquierdo(n);
    int indiceDerecho = derecho(n);
    int indiceCambio = indiceItercambiable(indiceIzquierdo, indiceDerecho);
    if (indiceCambio == -1)
      return;
    intercambia(n, indiceCambio);
    reordenaParaAbajo(indiceCambio, esMin);
  }
   ...
   * @return Valor del indice intercambiado.
   **/
  private int indiceItercambiable(int i, int j) {
    if ((0 <= i && i < this.arreglo.length) && (0 <= j && j < this.arreglo.length)) {
      if (arreglo[i].equals(arreglo[j]))
        return i;
      return j;
    }
    if (i < this.tamanio && j >= this.tamanio) {
      return i;
    }
    if (i >= this.tamanio && j < this.tamanio) {
      return j;
